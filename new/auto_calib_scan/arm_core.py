...
def detect_card_rect(detections: dict, card_tag_id: int, card_width_mm: float, card_height_mm: float,
                      H: np.ndarray) -> Optional[tuple[float, float, float, float, float]]:
    """Turns one AprilTag detection (stuck to a physical card) into a
    (center_x_mm, center_y_mm, width_mm, height_mm, rotation_deg) scan
    rectangle -- the exact same shape calib_scan_area()/scan_area_corners()/
    generate_scan_path() already use, so drawing and path generation don't
    need any new code, only this detection->rectangle conversion.

    `detections` is whatever arm_hardware.TagDetector.detect() returns --
    {tag_id: Detection}, Detection having .center (pixel) and .corners (4
    pixel points, in order). `card_width_mm`/`card_height_mm` are the
    card's own physical size (measured by hand, not derived from the tag
    itself -- the tag is much smaller than the card it's stuck to). `H` is
    the already-fit pixel->mm homography (calib["homography"]["H"], same
    single-plane assumption every other apply_homography() caller makes).

    Returns None if `card_tag_id` isn't in `detections` this frame (card
    not visible / out of frame / bad lighting) -- callers should keep
    showing the previous rectangle (or refuse to run) rather than treat
    this as a fatal error, since it can transiently happen frame to frame.

    ASSUMPTION: the tag is mounted so its own edges align with the card's
    edges (no separate tag-to-card rotation offset is calibrated) -- if a
    build glues the tag on crooked, rotation_deg will be off by that same
    fixed amount.
    """
...
@dataclass
class CardConfig:
    """Identifies the physical card card_gui.py scans: which AprilTag is
...
def _default_calib() -> dict:
    ...
    return {
        ...
        # Which AprilTag is stuck to the card card_gui.py scans, and the
        # card's own hand-measured physical size -- see CardConfig/
        # detect_card_rect. Optional, like hardware/motion: missing ->
        # calib_card_config() fills in these same defaults.
        "card": asdict(CardConfig()),
...
def _validate_calib(calib: dict) -> None:
    ...
    card = calib.get("card")
    if card is not None:
        # Optional, like hardware/motion -- calib_card_config() fills in
        # defaults for a fresh install / a calib.json that predates this
        # section, so it's not required here.
        tag_id = card.get("tag_id")
        if not isinstance(tag_id, int) or isinstance(tag_id, bool) or tag_id < 0:
            raise ValueError(f"calib.json card.tag_id={tag_id} must be a non-negative integer")
