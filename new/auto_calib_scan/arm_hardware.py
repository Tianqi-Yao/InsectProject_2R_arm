...
class Camera:
    """Wraps either picamera2 (Raspberry Pi CSI camera, the eventual
    deployment target) or a plain UVC/USB webcam via OpenCV's
    cv2.VideoCapture (for bench debugging on a dev machine before moving
    to the real Pi) behind the same connect()/close()/capture_gray()
    interface -- selected by calib.json's hardware.camera_backend, not a
    code change, so switching back to the Pi's camera later is a one-line
    config edit, not a code edit."""

    def __init__(self, resolution: tuple = (1920, 1080), backend: str = "picamera2",
                 usb_index: int = 0):
    ...
    def capture_gray(self):
        ...
        if self.backend == "usb":
            # A handful of consecutive UVC drivers (macOS included) drop
            # the first read() or two right after VideoCapture opens --
            # frames haven't started streaming yet even after connect()'s
            # warm-up sleep -- so a bare single read() is flaky. Retry a
...
class ArmHardware:
    ...
    def __init__(self, servo_port: str, joint_ids: dict, camera_resolution: tuple = (1920, 1080),
                 camera_backend: str = "picamera2", usb_camera_index: int = 0):
        self.servos = Servos(joint_ids)
        self.camera = Camera(camera_resolution, backend=camera_backend, usb_index=usb_camera_index)
