...
class ArmController:
    ...
    @property
    def dt(self) -> float:
        """Recomputed from self.motion_cfg.control_hz on every access
        (rather than cached at construction) so a live editor
        (control_gui.py's PARAMS mode) that mutates motion_cfg in place
        takes effect immediately, not just after a restart."""
        return 1.0 / self.motion_cfg.control_hz
    ...
    @property
    def goal_deg(self) -> TwoTuple:
        """The final destination of whatever segment is currently queued
        (or the last pose arrived at, if nothing is queued) -- NOT the
        same as commanded_deg, which is wherever the planner says the
        servos should be RIGHT NOW, mid-interpolation. Use this as the
        base for a caller building a "receding horizon" incremental jog
        (see nudge_workspace's docstring for that pattern): stepping from
        commanded_deg or a physically-measured present angle instead would
        under-accumulate while the arm is still catching up to the
        previous step, making a held jog key feel sticky instead of
        smooth."""
    ...
    def resync(self, j1_deg: float, j2_deg: float) -> None:
        ...
        torque re-engages following any period where the arm could have
        been hand-moved while free (e.g. a teach-in drag): self._commanded
        only ever gets updated from inside tick() while something is
        queued, so it silently goes stale the moment torque is released
        and the arm is dragged somewhere else by hand. Without this, the
        next goal-setting call plans a segment starting from that stale
        remembered pose, not from where the arm actually now is --
        streaming a phantom jump back through it before continuing on to
        the real target. Also drops any in-flight queue/scan, since both
        were planned against the same stale state."""
