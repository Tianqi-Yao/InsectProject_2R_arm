"""Minimal motion controller for teach: single-target trapezoidal
point-to-point moves, used by `goto` to replay to a previously hand-taught
point under torque. See kinematics_fit/controller.py for the twin of this
module and why each v2 feature package carries its own copy."""
...
class ArmController:
    ...
    def resync(self, j1_deg: float, j2_deg: float) -> None:
        """See kinematics_fit/controller.py's resync() docstring: this must
        be called after torque re-engages following any hand-move (which,
        in teach/, is the NORMAL case, not an edge case -- every `goto`
        follows a period of hand-teaching with torque off)."""
